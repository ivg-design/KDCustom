...
enum MCPTools {
    ...
    static let all: [MCPToolDefinition] = [
        ...
        tool("kdcustom_get_runtime", "runtime.get", "Get runtime",
             ...
             empty, readOnly: true),
        tool("kdcustom_test_rive_arrow_rate", "runtime.testRiveArrowRate", "Measure Rive arrow repeat rate",
             "Native Rive numeric arrows always use a 12 Hz limit. keyboard starts a five-minute filtered-detent measurement; stop ends measurement without disabling the permanent limit. Excess detents are discarded, never queued; reversals release the old arrow immediately and obey the same cap. Sends no input, changes no profiles, and does not disable detection or output guards. Status and counts appear in get_runtime.",
