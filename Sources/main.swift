...
final class SessionLog {
    ...
    init() throws {
        ...
        directory = URL(fileURLWithPath: repoPath).appendingPathComponent("evidence/capture-\(name)")
...
final class Probe: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        session.append(["kind": "session", "version": "0.2.0", "scope": "K40 USB and Bluetooth controls plus explicit label tests", "exclusive": false])
        openManager()
    ...
    func makeUI() {
        ...
        window.title = "K40 Feasibility Probe"
        ...
        stack.addArrangedSubview(text("K40 · controls and screen", size: 25, bold: true))
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "Quit K40 Probe", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
    ...
    func openManager() {
        ...
        manager = m
        // Both identities were observed locally. BLE vendor data uses FFE2,
        ...
        else if result == kIOReturnNotPermitted { health = "Input Monitoring required · enable K40 Probe, then quit and reopen it" }
