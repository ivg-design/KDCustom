...
### Temporary Rive arrow-rate comparison

For an explicitly requested focus-loss diagnostic, `kdcustom_test_rive_arrow_rate` with `mode: "keyboard"` limits the existing verified Rive numeric-arrow path to 12 downs per second for five minutes. It sends no events on its own, keeps automatic area detection and output guards active, and changes no profiles. Excess physical detents are discarded, not queued; reversals release the old arrow immediately and respect the same cap. `mode: "stop"` restores normal delivery early. Normal delivery also returns automatically on expiration or app restart.

The app status shows **Ready · Rive 12 Hz test** while active. `kdcustom_get_runtime.riveArrowRateTest` reports `active`, `remainingSeconds`, `filteredDetents`, and `repeatLimitHz` (null in normal mode). Start a separate bounded panel capture to compare emitted and HID-observed event timestamps and focus changes. A stable private or fixture test is not live Rive acceptance. This temporary mutation is annotated non-read-only and cannot be included in profile-edit batches.
