...
When automatic Rive area detection selects numeric shortcuts, build 35 limits native arrow output to 12 presses per second. The user confirmed that values changed at a usable speed and focus did not drop during the capped trial. Slow turns retain individual increments; excess fast detents are skipped rather than replayed after you stop. Direction and modifier changes release the old arrow before continuing at the same limit. This is permanent across app restarts and does not change canvas zoom, timeline stepping, buttons, or other app output.
