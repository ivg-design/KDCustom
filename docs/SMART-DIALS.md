...
- **Tab → Shift+Tab · keep editing** applies with Tab, then reverses traversal. It checks the same app/window, an intermediate nonsecure text control and return to the original field with the expected value. The user confirmed this manually in Rive; integrated dial acceptance remains separate.
