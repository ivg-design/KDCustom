...
- **Tab → Shift+Tab · keep editing** applies with Tab, then reverses traversal. It checks the same app/window, an intermediate nonsecure text control at a distinct position and return to the original field bounds with the expected value. Geometry is needed when an app reuses one native editing element across fields. The user confirmed this manually in Rive, but build 24's integrated test stopped after Tab; it required a different AX element before sending Shift+Tab. The geometry-based correction is awaiting a physical retest. Keep manual apply for the confirmed workflow.
