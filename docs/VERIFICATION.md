...
Focused arrow-state and MCP tests cover rate enforcement, no deferred output, reversal/context cancellation, unchanged normal mode, non-read-only tool metadata, and rejection of arbitrary output/duration parameters.

Build 34 was signed, notarized, installed after verified normal exit, and checked against the built executable. All 14 profiles remained byte-identical across installation, launch, and diagnostic setup. USB and all permissions were ready; all 20 MCP tools were discoverable and the rate-test start/stop checks passed.

During the subsequent physical trial, the user reported being unable to cause focus loss. The rate test was still active when the result was captured, with 1,529 excess detents filtered across the trial. The retained 1,200-event ring covers only the final 19.85 seconds, not the complete trial. It contains 49 posted arrow downs (25 Up, 24 Down), matched by 49 marked downs at the HID observer. Posted intervals were at least 83.87 ms, with a 93.79 ms median. No field-to-container focus exit was retained before switching applications. This supports a rate-related contribution, but does not establish Rive's internal cause, long-term reliability, or exact numeric values.

HID-observer callback timestamps had a 94.63 ms median interval and a 66.51 ms minimum. These are callback logging times, not Rive handler timestamps; they do not demonstrate a strict 83.3 ms minimum at the receiver. The user's no-drop result is distinct from that timing limitation. After saving the result, the temporary rate mode and capture were stopped and runtime returned to Ready/unpaused. Whether the capped speed is usable remains a user-feedback item. No permanent rate setting or profile change has been applied.

## Build 35: accepted Rive rate becomes the default

The user subsequently confirmed that values changed at a usable speed and requested keeping that rate. Build 35 makes the same 12 Hz posting limit permanent for the verified Rive numeric-arrow path, including both directions and configured modifiers. No profile migration is needed. Canvas, timeline, buttons and other app output paths are unchanged. Fast excess detents are discarded, not replayed after stopping; context or direction changes cannot bypass the cap.

The platform entry point always uses the core's 12 Hz default, independent of any diagnostic timer. The existing rate-test MCP tool now measures filtered detents for five minutes; stopping or expiring measurement cannot disable pacing. Runtime reports the permanent rate and total filtered count separately from the bounded measurement. Focused arrow-state tests passed for default pacing after fresh state and beyond the old timeout, reversal/context release, modifier preservation and no catch-up. MCP validation tests also passed. The live physical acceptance is the user's build-34 trial, not an additional build-35 physical test.

The warnings-as-errors native build passed. Build 35 was Developer ID signed, notarized, stapled, and Gatekeeper accepted, then installed after normal Quit and verified process exit. The installed executable matches the verified build. All 14 profiles remained byte-identical across installation, relaunch, and MCP checks. Live runtime is Ready/unpaused with USB ready and all permissions allowed. All 20 tools are available; the permanent 12 Hz limit is reported immediately after launch with measurement inactive, and remains 12 Hz after the diagnostic start/stop check. No agent-generated input was sent to Rive during installation verification.
