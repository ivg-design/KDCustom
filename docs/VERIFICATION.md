...
- Build 26 is installed, Developer ID signed, notarized, stapled and accepted by Gatekeeper after a normal quit. All 14 profiles are byte-identical to the snapshot before the build 25/26 updates. USB is ready, all three permissions remain allowed, and the installed MCP helper exposes 19 tools. No Rive mapping was changed during these updates.
- All 21 focused suites pass, including field geometry and detached Flutter editor/panel fixtures. The native build passes with warnings as errors. Fixtures do not establish Rive focus return or panel detection.
- Rive Group 2 gives both dials the same independently configurable mixed rules: native arrows for ±1, Command+arrows for ±0.1, Shift+arrows for ±10, typed numeric input with Control for ±0.01 and Control+Shift for ±100. Clockwise decreases; counterclockwise increases. The user's Control selector is preserved. All four numeric bindings currently use **manual apply**.
...
- Build 25 replaces the different-AX-element requirement with geometry checks, anchors the original field before writing, and defers its own focus notifications during the bounded transaction. The app/window, actual focus, nonsecure role, numeric value, physical-input cancellation and timeout checks remain. **Physical Rive acceptance is pending.**
- The panel probe now discovers shallow Flutter semantic siblings around detached native text fields and uses only local built-in chrome tokens plus bounds. It retains no raw document text or numeric values. Panel classification is diagnostic only; automatic hierarchy/timeline/canvas routing is not enabled or accepted.
