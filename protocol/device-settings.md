...
| Rotation | `DE`, `QueryRotateDegree` `0x100063058` | Payload byte 0 `00/01/02/03` → `0°/90°/180°/270°`; values above `03` → 0 in the driver. Live evidence below establishes `04` as full-turn/0° on this K40. | `DD` advances one step, `RotateDegree` `0x100062ff0`. It discards the reply. The live cycle and UI orientation correction are recorded below. |
